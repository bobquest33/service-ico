...
def setup_workers(ctx, config):
    ...
    workers_dict = get_workers('etc/k8s/{}/workers.json'.format(config_dict['NAMESPACE']))

    ctx.run('kubectl apply -f etc/k8s/{}/workers.yaml'.format(config_dict['NAMESPACE']))
...
def manage(ctx, config, version_tag, cmd):
    ...
    env = '\"DEBUG=FALSE,POSTGRES_DB={postgres_db},POSTGRES_USER={postgres_user},POSTGRES_PASSWORD={postgres_password}\"'.format(postgres_db='postgres',
